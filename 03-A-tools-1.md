...
audio tools on the same weave. You connect one or more tools to the sync tool,
and the tool itself is a button that allows the reader to start and stop
playback of all connected tools. This can, for example, be used to document
video installations with multiple screens and allows playing back synchronized
combinations of musical voices and audio tracks. 

### Media

The sync tool itself is a button, but you have to bring your own icons for the state of the button. In the media tab you can upload two images for this. One is for the "play" button, starting the playback. The other image represents stopping the playback in all connected tools.

### Options

In the options tab you can sellect which tools need to be connected to this Sync
Tool. Only tools already present in the current weave may be selected. The option “pause other
players when playing” will ensure that only the selected media files will be
played back.
